<style lang="less">
    ...
    .wrapper-in{ height: 100%; padding-top: 70px;}

    .sidebar{ float: left; width: 240px; height: 100%; border-radius: 0; overflow-x: scroll;}

    /*.contain{ width: auto; height: 100%; overflow-x: hidden;}*/
    .contain{ width: auto; padding: 30px 50px; height: 100%; overflow-x: hidden;}
    .contain-title{ margin-bottom: 30px; padding: 0 20px; border-bottom: 1px solid #ddd; font-size: 18px; color: #333; line-height: 60px;}
    .contain-bottom-pagination{ padding: 20px 0; overflow: hidden;}

    .header{ position: fixed; top: 0; left: 0; width: 100%; height: 70px; padding: 0 30px; background-color: #324157; color: #bfcbd9; line-height: 70px;
        &-logo{ height: 75%; vertical-align: middle; cursor: pointer;
            div&{ width: 210px;}
        }
        &-park{
            &-change{ text-decoration: underline; cursor: pointer;}
        }
    }
    .header-admin{ font-size: 14px;
        .el-dropdown{ color: #bfcbd9;
            &-link{ cursor: pointer;}
        }
    }
    .header-logout,.header-logout{ text-decoration: none;}

    .scoped-pointer{ cursor: pointer;}
</style>
<template>
    <div class="wrapper">
        ...
        <el-dialog
            title="修改密码"
            :visible.sync="passwordChangeDialogVisible"
            size="tiny"
            @close="handleResetPasswordChange">
            <el-form 
                :model="passwordChangeFormData"
                :rules="passwordChangeFormRules"
        ...
        <el-dialog
            title="修改手机号"
            :visible.sync="cellphoneChangeDialogVisible"
            size="tiny"
            @close="handleResetCellphoneChange">
            <el-form 
                :model="cellphoneChangeFormData"
                :rules="cellphoneChangeFormRules"
                ref="cellphoneChangeForm"
            >
                <el-form-item prop="oldCellphone">
                    <el-input
                        type="cellphone"
                        v-model="cellphoneChangeFormData.oldCellphone"
                        placeholder="请输入老手机号"
                    ></el-input>
                </el-form-item>
                <el-form-item prop="cellphone">
                    <el-input
                        type="cellphone"
                        v-model="cellphoneChangeFormData.cellphone"
                        placeholder="请输入新手机号"
                    ></el-input>
                    <el-button type="primary" size="small"
                        :disabled="getSmscodeDisabled" @click="getSmsCode">{{ getSmscodeButtonWord }}</el-button>
                </el-form-item>
                <el-form-item prop="smsCode">
                    <el-input
                        v-model="cellphoneChangeFormData.smsCode"
                        placeholder="请输入验证码"
                    ></el-input>
                </el-form-item>
            </el-form>
    ...
    export default {
                ...
                for (let i = 0; i < navMenu.length; i ++) {
                    if (Object.prototype.hasOwnProperty.call(navMenu[i], 'children') === false) {
                        ...
                        if (navMenu[i].permissions === 0) {
                            ...
                            if (this.$store.state.isParkManager === true || this.$store.state.isSuper === true) {
                                // 园区经理
                                navMenu[i].permit = true;
                            } else {
                                // 判断权限
        ...
        methods: {
            activePathCheck(path) {
                ...
                // const indexRouteReg = /^\/manageSystem\/index/;
                // const propertyRoomRouteReg = /^\/manageSystem\/property\/room/;
                // const propertyRepairmentRouteReg = /^\/manageSystem\/property\/repairment/;
                // const propertyComplaintRouteReg = /^\/manageSystem\/property\/complaint/;
                // const propertyActivityRouteReg = /^\/manageSystem\/property\/activity/;
                // const mallMerchandiseRouteReg = /^\/manageSystem\/mall\/merchandise/;
                // const mallOrderRouteReg = /^\/manageSystem\/mall\/order/;
                // const mallCouponRouteReg = /^\/manageSystem\/mall\/coupon/;
                // const managerDepartmentRouteReg = /^\/manageSystem\/manager\/department/;
                // const managerRouteReg = /^\/manageSystem\/manager\/manager/;
                // const managerLogsRouteReg = /^\/manageSystem\/manager\/logs/;
                // const systemExceptionRouteReg = /^\/manageSystem\/system\/exception/;
                // const systemAccessRouteReg = /^\/manageSystem\/system\/access/;
                // const systemSmsRouteReg = /^\/manageSystem\/system\/sms/;
                // const systemParkRouteReg = /^\/manageSystem\/system\/park/;
                // const userOperationLogsRouteReg = /^\/manageSystem\/user\/operationLogs/;

                // if (indexRouteReg.test(path) === true) {
                //     this.activeIndex = '/manageSystem/index'
                // } else if (propertyRoomRouteReg.test(path) === true) {
                //     this.activeIndex = '/manageSystem/property/room';
                // } else if (propertyRepairmentRouteReg.test(path) === true) {
                //     this.activeIndex = '/manageSystem/property/repairment';
                // } else if (propertyComplaintRouteReg.test(path) === true) {
                //     this.activeIndex = '/manageSystem/property/complaint';
                // } else if (propertyActivityRouteReg.test(path) === true) {
                //     this.activeIndex = '/manageSystem/property/activity';
                // } else if (mallMerchandiseRouteReg.test(path) === true) {
                //     this.activeIndex = '/manageSystem/mall/merchandise';
                // } else if (mallOrderRouteReg.test(path) === true) {
                //     this.activeIndex = '/manageSystem/mall/order';
                // } else if (mallCouponRouteReg.test(path) === true) {
                //     this.activeIndex = '/manageSystem/mall/coupon';
                // } else if (managerDepartmentRouteReg.test(path) === true) {
                //     this.activeIndex = '/manageSystem/manager/department';
                // } else if (managerRouteReg.test(path) === true) {
                //     this.activeIndex = '/manageSystem/manager/manager';
                // } else if (managerLogsRouteReg.test(path) === true) {
                //     this.activeIndex = '/manageSystem/manager/logs';
                // } else if (systemExceptionRouteReg.test(path) === true) {
                //     this.activeIndex = '/manageSystem/system/exception';
                // } else if (systemAccessRouteReg.test(path) === true) {
                //     this.activeIndex = '/manageSystem/system/access';
                // } else if (systemSmsRouteReg.test(path) === true) {
                //     this.activeIndex = '/manageSystem/system/sms';
                // } else if (systemParkRouteReg.test(path) === true) {
                //     this.activeIndex = '/manageSystem/system/park';
                // } else if (userOperationLogsRouteReg.test(path) === true) {
                //     this.activeIndex = '/manageSystem/user/operationLogs';
                // }
            },
            getParkList(bool) {
                this.$http({
            ...
            changePasswordMessageBox() {
                // 被坑了  多个messagebox弹出的时候层级会出问题   用dialog
                // const _this = this;
                ...
                //             // ..
                //         } else {
                //             _this.passwordChangeFormData.oldPassword = '';
                //             _this.passwordChangeFormData.newPassword = '';
                //         }
                //     },
                //     beforeClose(action, instance, done) {
                //         if (action === 'confirm') {
